...
SEED = 323
base_dir = '../input/'
def seed_everything(seed=SEED): img_name_short = self.image_files_list[idx].split('.')[0]
    random.seed(seed)
...
for batch_i, (data, target) in enumerate(test_loader):
    data, target = data.cuda(), target.cuda()
    output = mod img_name_short = self.image_files_list[idx].split('.')[0]

    pr = output[ img_name_short = self.image_files_list[idx].split('.')[0]
    for i in pr: img_name_short = self.image_files_list[idx].split('.')[0]
        preds.append(i)
